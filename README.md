...
src/
  ...
  shared/
    ...
    lib/
      mock-data/     # Seeded pseudo-random generators (mulberry32) + deterministic datasets
...
All datasets, schemas, quality metrics, pipelines, alerts and dashboard metrics are generated by a seeded PRNG (`mulberry32`), so the same seed always produces the same data — useful for demos, screenshots and tests. Row-level data for the Dataset Explorer is generated and cached per dataset/row-count pair to keep the virtualized grid fast at 30k+ rows. A `withLatency()` helper simulates realistic network delay on every mock API call.
...
This runs the multi-stage `Dockerfile` (deps → build → minimal Alpine runtime, non-root user) and exposes the app on port 3000. Point `env_file` at a populated `.env.local` before running in an environment where the AI integration is live.
